...
namespace Mageplaza\OrderExport\Block\Liquid\Tag;
...
class TagFor extends AbstractBlock
{
    ...
    private function renderCollection(Context $context)
    {
        if ($this->collectionName === 'collection') {
            return $this->renderProfileCollection($context);
        }
     ...
     * @throws FileSystemException
     */
    private function renderProfileCollection(Context $context)
    {
        $objectManager = ObjectManager::getInstance();
        /** @var Data $helperData */
        $helperData = $objectManager->get(Data::class);
        $feedId     = $context->get('profile_id');

        $resultAll = '';
        $index     = 0;
        $length    = $helperData->getProfileSessionData($feedId, 'object_count');
        foreach ($helperData->getProfileCollectionPaths($context->get('profile_id')) as $path) {
            $collection = Data::jsonDecode($helperData->readFile($path))['items'];
